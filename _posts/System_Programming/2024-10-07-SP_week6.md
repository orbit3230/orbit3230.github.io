...
excerpt: "Assembly Basics, x86-64 Integer Registers, Moving Data, Simple Memory Addressing Modes, Complete Memory Addressing Modes, AT&T vs Intel syntax, Arithmetic & Logical Operations, Address Computation Instruction, Contidion Codes, Jumping, Conditional branches"
...
- Bad Cases for Conditional Move
  ...
  `val = Tes(x) ? Hard1(x) : Hard2(x);`
    - 각각이 많은 연산을 다루는 경우이다.  
    - Conditional Move는 두 연산을 모두 수행하므로, 이럴 바에는 Branches를 사용하는 것이 낫다.  
  - (2) Risky Computations  
  `val = p ? *p : 0;`
    - `p`가 `NULL`인 경우, `*p`를 수행하면 segmentation fault가 발생한다.  
    - 의도하지 않은 effects가 발생할 수 있다.  
...
![jump_instruction_encodings](TODO)  
