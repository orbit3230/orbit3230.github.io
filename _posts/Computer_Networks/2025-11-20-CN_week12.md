  ...
  - Routing to another subnet
    - e.g., `A`는 `B`에게 datagram을 보내길 원함. 이 때, `R`이라는 라우터가 `A`와 `B` 사이에 있음  
      ...
      - 가정하길,
        ...
        - `A`는 first hot 라우터 `R`의 IP 주소를 알고 있음  
        - 따라서 `A`는 `R`의 MAC 주소도 알 수 있음  
      ![arp_routing_process_1](TODO)  
...
- Physical Topology of Ethernet  
  - Bus  
  ![ethernet_bus_topology](TODO)
    - 90년대 중반까지 popular
    - 모든 노드가 같은 collision domain에 속함 (서로 충돌 가능)

  - Switched  
  ![ethernet_switched_topology](TODO)  
    - 오늘날 만연히 사용됨
