...
![bgp_path_advertisement_1][def8]  
  ...
  - `AS2` policy에 기반하여, `AS2` 라우터 `2a`는 eBGP를 통해 path `AS2, AS3, X`를 `AS1` 라우터 `1c`에게 광고  

![bgp_path_advertisement_2][def9]
  - `AS1` 게이트웨이 라우터 `1c`는 `AS2, AS3, X` 를 `2a`로부터 학습
  - `AS1` 게이트웨이 라우터 `1c`는 `AS3, X`를 `3a`로부터 학습
  - **policy**에 기반하여, `AS1` 게이트웨이 라우터 `1c`는 `AS3, X` path를 선택하고 iBGP를 통해 `AS1` 내 모든 라우터들에게 propagate
...
- BGP messages
  ...
  - BGP 메시지 유형
    - `OPEN`
      - BGP peer를 remote하기 위해 TCP 연결을 열고, BGP peer를 인증  
    - `UPDATE`
      - 새로운 path를 advertise (또는 오래된 것을 withdraw)
    - `KEEPALIVE`
      - `UPDATE` 부재 시 연결이 살아있음을 확인
      - 또한 peer로부터 `OPEN` 메시지에 응답(ack)하기 위해 사용  
      (즉, `OPEN` 메시지에 대한 응답으로 `KEEPALIVE` 메시지를 보냄)  
    - `NOTIFICATION`
      - 이전 메세지에 에러가 있음을 알림
      - 또한 연결을 종료하기 위해 사용  
...
![bgp_path_advertisement_3][def10]  
  - `1a`, `1b`, `1d`는 iBGP를 통해 `1c`로부터 "`1c`를 통해 `X`에 도달할 수 있다"는 것을 학습
  - `1d` 에서
    - iBGP가 말하길 : `X`에 가려면, 인터페이스 `1`를 사용해
    - OSPF intra-domain 라우팅이 말하길 : `1c`에 가려면, 인터페이스 `1`를 사용해  
![bgp_path_advertisement_4][def11]  
  - `1a`, `1b`, `1d`는 iBGP를 통해 `1c`로부터 "`1c`를 통해 `X`에 도달할 수 있다"는 것을 학습
  - `1a` 에서
    - iBGP가 말하길 : `X`에 가려면, 인터페이스 `2`를 사용해
    - OSPF intra-domain 라우팅이 말하길 : `1c`에 가려면, 인터페이스 `2`를 사용해  
...
- Hot potato routing  
![hot_potato_routing][def12]  
  - 특정 목적지로 가는 여러 경로가 있을 때, 가장 적은 intra-domain cost를 가지는 local gateway를 선택
  (e.g., `2d`는 `2a`를 선택. 비록 `X`로의 더 많은 hop이 걸리더라도)
  - inter-domain cost에 대해서는 걱정하지 않는다.  

<br>

- BGP: Implementing policy via advertisement  
  - ISP는 오직 트래픽이 자기들 고객의 네트워크로만 오고 가길 원함
    - 다른 ISP간의 transit traffic을 운반하길 원하지 않음
    - 일반적인 real-world policy  

  - Example Scenario 1  
  ![bgp_implementing_policy_via_advertisement_1][def13]
    - `A`는 path `A, w`를 `B`와 `C`에게 광고
    - `B`는 `B, A, w`를 `C`에게 광고하지 않기로 결정 (policy)
      - `B`는 `C`->`B`->`A`->`w` 라우팅에 대한 아무런 수익이 없음. 왜냐면 `C`, `A`, `w` 모두 `B`의 고객이 아니기 때문
      - `C`는 `C, B, A, w` path에 대한 어떠한 정보도 받지 못함  

    - Example Scenario 2  
    ![bgp_implementing_policy_via_advertisement_2][def14]
      - `A`, `B`, `C`는 provider networks
      - `x`, `w`, `y`는 customers
      - `x`는 dual-homed (두 개의 네트워크에 연결됨)
      - **Policy to enforce** : `x`는 `B`에서 `C`로 가는 트래픽을 허용하지 않음
        - `x`는 `B`에게 `C`로 가는 path를 광고하지 않음

<br>

- BGP route selection  
  - 라우터는 destination AS로 가는 하나 이상의 route를 학습할 수도 있음

  - Route Selection은 아래와 같은 근거로 수행됨
...
[def6]: https://i.imgur.com/KDhOnQ6.png
[def7]: https://i.imgur.com/7rzDyQY.png
[def8]: https://i.imgur.com/2U2cXs8.png
[def9]: https://i.imgur.com/8OnLL2y.png
[def10]: https://i.imgur.com/u9YO6Kz.png
[def11]: https://i.imgur.com/4xESfHM.png
[def12]: https://i.imgur.com/WTvnKTU.png
[def13]: https://i.imgur.com/fc8orCc.png
[def14]: https://i.imgur.com/M3X4m4T.png