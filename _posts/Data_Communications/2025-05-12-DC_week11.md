...
- **rdt 3.0: pipelined protocols operation**
  ...
  - Selective repeath: the approach
    ...
    - Sender는,
      ...
      - `N`개의 연속된 sequence number에 대해 "window"를 설정
        - window내 "in-flight" 패킷에 대해 제한  

    - Selective repeat: sender, receiver windows  
    ![Selective repeat sender, receiver windows](TODO)  
    ...
    - Selective repeat: sender
      ...
      - `ACK(n)` in `[sendbase, sendbase + N-1]`
        - `n`번째 패킷이 수신되었음을 표시
        - window base를 다음번 unACKed 패킷으로 이동, 만약 `n`이 가장 작은 sequence number의  unACKed 패킷이라면  
  ...
  ![Selective repeat in action](TODO)  
