...
- Congestion avoidance phase (Reno)  
  - 각 ACK에 대해, `CWND` <- `CWND + 1/CWND` (packets)  
...
- 만약 더 많은 duplicate ACKs를 받으면,  
  - 각 duplicate ACK에 대해  
    - `CWND` <- `ssthresh + 1`  
  - 각 ACK에 대해 (lost packet에 대한 retransmission)  
    - `CWND` <- `ssthresh`  

<br>

- Summary  
![TCP Congestion Control Summary](TODO)  
