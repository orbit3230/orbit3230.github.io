...
![Local DNS name servers](TODO)  
  - 호스트가 DNS query를 하면, local DNS server로 보내진다.  
    ...
    - Local DNS server는 hierarchy에 엄격하게 따라가지 않는다.  
      - Iterative Query  
      ![Iterative Query](TODO)
        - root DNS sserver에 부하가 덜 하다.
      - Recursive Query  
      ![Recursive Query](TODO)  
...
![DNS records](TODO)  
  ...
  - type = `MX`
    - name : **도메인** 
    - value : 이 도메인에 대한 **SMTP 메일 서버의 호스트 이름름**  

    <br>

- **DNS protocol messages**  
- DNS는 메시지를 query하고 reply하며, 두 경우 모두 같은 포맷을 사용한다.  
![DNS protocol messages_1](TODO)  
![DNS protocol messages_2](TODO)  
  
<br>

- **From IP address to name**  
![From IP address to name](TODO)  
  - RR type : `PTR`
  - **in-addr.arpa** 도메인
