...
![intermediate_object_mapping](TODO)  
...
- `s`와 `t`가 범위 `0`...`1` 내에 있길 원한다면, **clamping**이나 **repeat**을 사용하여 해당 범위(`[0, 1]`) 내에 있도록 강제할 수 있다.  
  ...
  - **Clamping** : 만약 `s`, `t` > `1` 이면 `1`, 그렇치 않으면 `0`  
  `glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP)`  
  - **Repeat** : 만약 `s`, `t` > `1` 이면 `modulo 1`  
  `glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT)`  
  - 비교  
  ![clamping_vs_repeat](TODO)  
...
![magnification_minification_1](TODO)  
