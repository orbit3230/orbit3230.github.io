...
반드시 하나의 수식적 제약을 만족해야한다.  
  ...
  `x`<sub>`2`</sub><sup>`T`</sup>`Fx`<sub>`1`</sub> = `0`  
  ...
  - 여기서 `F` = `K`<sub>`2`</sub><sup>`-T`</sup>`S`<sub>`t`</sub>`RK`<sub>`1`</sub><sup>`-1`</sup>  
...
`x`<sub>`2`</sub><sup>`T`</sup>`Fx`<sub>`1`</sub> = `l`<sub>`1`</sub><sup>`T`</sup>`x`<sub>`1`</sub> = `0`  
...
- Epipolar constraint `x`<sub>`2`</sub><sup>`T`</sup>`Fx`<sub>`1`</sub> = `0`는 선형 시스템(linear system)으로 다음과 같이 쓸 수 있다.  
![epipolar_constraint_linear_system][def9]  
  - `x`<sub>`1`</sub> = `[x`<sub>`1`</sub>, `y`<sub>`1`</sub>, `w`<sub>`1`</sub>`]` (Homogeneous coordinates)
  - `x`<sub>`2`</sub> = `[x`<sub>`2`</sub>, `y`<sub>`2`</sub>, `w`<sub>`2`</sub>`]` (Homogeneous coordinates)
  - 이렇게 두고 Epipolar constraint `x`<sub>`2`</sub><sup>`T`</sup>`Fx`<sub>`1`</sub> = `0`를 전개해보면,  
  ![epipolar_constraint_expanded][def10]  

- Fundamental matrix `F`의 원소는 `9`개이지만, scale은 arbitrary하기 때문에 실제로는 `9`-`1`=`8`개의 자유도가 존재한다.  
  - scale이 arbitrary하다는 것은, `F`의 모든 원소에 동일한 상수를 곱해도 Epipolar constraint가 여전히 만족된다는 것을 의미한다.  즉, `F` ∼ `λF` (λ는 임의의 스칼라)  

- 따라서, 오직 `8`개의 equation이 있으면 `Af` = `0` linear system을 풀어서 `F`를 계산할 수 있다.  
`8`개의 대응점이 필요하기 때문에 **Eight Point Algorithm**이다.  

<br>

- `∥Af∥`를 최소화하는 `f`를 계산하기 위한 Eight Point Algorithm 함수를 작성해보자.  
...
- `F`가 노이즈로 인해 rank `2`가 아닐 수 있기 때문에, 가장 작은 singular value `S[2]`를 `0`으로 만들어서 rank `2`로 강제한다.
  - 유용한 standard trick이다.  

<br>

- epipole은 `Fe`<sub>`1`</sub> = `0`을 만족해야 하며, `F`의 null vector로 계산할 수 있다.  
...
- sample data set을 사용하여 위 두 함수를 테스트해보자.  

```py
import load_vggdata as data
import sfm
from numpy import vstack, ones
from matplotlib.pyplot import figure, imshow, axis, plot, show

# index for points in first two views
ndx = (data.correspondences[:,0]>=0) & (data.correspondences[:,1]>=0)

# get coordinates and make homogeneous
x1 = data.points2D[0][:,data.correspondences[ndx,0]]
x1 = vstack((x1,ones(x1.shape[1])) )
x2 = data.points2D[1][:,data.correspondences[ndx,1]]
...
for i in range(5) :
    sfm.plot_epipolar_line(data.image1, F, x2[:,i], e, False)
axis('off')
...
![epipolar_lines_and_points][def11]  

- helper function인 `plot_epipolar_line()`은 다음과 같다.  
...
[def9]: https://i.imgur.com/SfPQnYl.png
[def10]: https://i.imgur.com/BlKMorA.png
[def11]: https://i.imgur.com/kH1SZm7.png