...
- principle of optimality란,  
만약 어떠한 instance에 대한 optimal solution이 되려면  
...
- e.g., shortest path problem
  ...
  subpath인 `v`<sub>`i`</sub> ~ `v`<sub>`k`</sub> 와 `v`<sub>`k`</sub> ~ `v`<sub>`j`</sub> 또한 optimal 해야만 한다.  
  - Subpaths 들이 어떠한 최적의 경로라면, 이들을 합친 combined paths 들 또한 optimal 할 것이다.  
...
![longest_paths_problem_example](TODO)    
...
- matrix multiplication의 결합 법칙에 따라, 어느 연산을 먼저 하느냐에 따라서 연산 횟수는 달라진다.  
...
- 이 문제에 principle of optimality가 적용된다.  
- e.g.,
