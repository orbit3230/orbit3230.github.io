...
- Dynamic Programming을 적용해보자.  
  ...
  ![element_of_D][def21]  
  <br>
  - 가장 Easy Problem 부터 해결해 나가야 하므로, 종착지인 `v`<sub>`1`</sub>에서 이전 노드를 하나씩 이어나가며 최단 경로를 찾아나가는 형식으로 문제를 해결한다.  
    - 가령 `v`<sub>`1`</sub> ~ `v`<sub>`4`</sub>이 있을 때,  
    
    - <2 Elements>  
    `v`<sub>`2`</sub>->`v`<sub>`1`</sub>  
    `v`<sub>`3`</sub>->`v`<sub>`1`</sub>  
    `v`<sub>`4`</sub>->`v`<sub>`1`</sub>  
    를 저장해둔다.  

    - <3 Elements>  
    min(`v`<sub>`2`</sub>->`v`<sub>`3`</sub>->`v`<sub>`1`</sub>, `v`<sub>`2`</sub>->, `v`<sub>`4`</sub>->`v`<sub>`1`</sub>)  
    min(`v`<sub>`3`</sub>->`v`<sub>`2`</sub>->`v`<sub>`1`</sub>, `v`<sub>`3`</sub>->, `v`<sub>`4`</sub>->`v`<sub>`1`</sub>)  
    min(`v`<sub>`4`</sub>->`v`<sub>`2`</sub>->`v`<sub>`1`</sub>, `v`<sub>`4`</sub>->, `v`<sub>`3`</sub>->`v`<sub>`1`</sub>)  
    총 3가지 sub path를 저장한다.  
    이렇게되면 결국 {2, 3}, {2, 4}, {3, 4} 간의 순서가 정립된 것이다.  
