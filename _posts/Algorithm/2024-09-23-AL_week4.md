...
![binary_search_tree_example][def16]  
  ...
  - 따라서 **평균적인 탐색 시간을 최소화**하도록 binary serach tree의 **key들을 organize** 하는 것이 목표이다.  
    ...
    ![binary_search_tree_average_search_tree](TODO)  
    를 최적화 하는 것이 목표이다.  
  
  - 여기에 ***Dynamic Programming***이 이용될 수 있다.  

- 배열의 `A[i][j]`의 정의를, `i` 번째 ~ `j` 번째 까지 노드의 Average search time 이라고 해보자.  

- principle of optimality가 적용되기 때문에,  
`k`번째 `key` -> `key`<sub>`k`</sub>를 루트에 배치한 것이 최적의 방법이라면,  
left subtree의 Average search time인 `A[1][k-1]` 는 optimal 할 것이고,  
right subtree의 Average search time인 `A[k+1][n]` 또한 optimal 할 것이다.  
  ...
  ![average_search_time_from_1_to_n](TODO)  
