...
- 이 과정을 Dynamic Programming 으로 해결해볼 수 있다.  
  - `D`<sup>`(k-1)`</sup>으로부터 `D`<sup>`(k)`</sup>를 계산하도록 재귀적 성질을 구축한다.  
    - 오직 `{v`<sub>`1`</sub>`, v`<sub>`2`</sub>`, ... , v`<sub>`k`</sub>`}`의 vertice 만을 가지고 `v`<sub>`i`</sub> 부터 `v`<sub>`j`</sub>까지 가는 최단 경로가 **하나라도 존재**한다면,  
    `v`<sub>`k`</sub>를 **사용하지 않는다**.  

    ![not_using_v_k][def8]

    - 만약 그렇지 않다면, `v`<sub>`k`</sub>를 **사용한다**.  
