...
- Banker's Algorithm의 Request Algorithm
  - `Request_i` = 프로세스 `Pi`에 대한 request vector. 만약 `Request_i[j] = k`라면, 프로세스 `Pi`가 리소스 타입 `Rj`의 `k`개 인스턴스를 요청하고 있다는 의미
    ...
    - (3) 다음과 같은 상태를 수정함으로써 프로세스 `Pi`가 요청한 리소스를 할당했다고 가정한다.  
...
- Example of Banker's Algorithm  
  ...
  - `T0` 시간에서의 스냅샷  
  ![Banker's Algorithm_T0](TODO)  

  - `Need` Matrix의 내용은 `Max - Allocation`으로 계산된다.  
  ![Banker's Algorithm_Need Matrix](TODO)  
  ...
  - Initialize  
  ![Banker's Algorithm_Initialize](TODO)  

  - `i = 0`  
  ![Banker's Algorithm_i=0](TODO)  

  - `i = 1`  
  ![Banker's Algorithm_i=1](TODO)  

  - `i = 2`  
  ![Banker's Algorithm_i=2](TODO)  

  - `i = 3`  
  ![Banker's Algorithm_i=3](TODO)  

  - `i = 4`  
  ![Banker's Algorithm_i=4](TODO)  

  - `i = 0` again  
  ![Banker's Algorithm_i=0 again](TODO)  

  - `i = 2` again  
  ![Banker's Algorithm_i=2 again](TODO)  
...
- **Single Instance of Each Resource Type**  
  ...
  ![Resource-allocation graph and wait-for graph](TODO)  
