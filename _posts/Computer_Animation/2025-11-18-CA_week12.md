...
- Iterative Approaches  
  ...
  - Gauss-Newton 방식을 사용하여 오차를 최소화하는 `Δθ`를 구할 수 있다.
    ...
    ![inverse_kinematics_iterative_3](TODO)  
      - 우리가 알고 있는 것은 `θ`<sup>`k`</sup>와 `c` (specified position) 이다.  
      테일러 1차 근사를 사용하여 식을 다시 써보면,  
      ![inverse_kinematics_iterative_4](TODO)  
      ![inverse_kinematics_iterative_5](TODO)  

    - Gradient가 `0`일 때 optimal value를 가진다.  
    `J Δθ` = `b` 형태의 Linear System을 풀면 된다.  
    ![inverse_kinematics_iterative_6](TODO)  
      ...
      - 일반적으로 `m` < `n` 이다. 이는 Under-determined system이다. 따라서 pseudo-inverse는 `(J`<sup>`T`</sup>`J)`<sup>`-1`</sup>`J`<sup>`T`</sup> 가 아닌 `(JJ`<sup>`T`</sup>`)`<sup>`-1`</sup>`J`<sup>`T`</sup> 를 사용한다.  
