...
- Iterative Approaches  
  ...
  - Gauss-Newton 방식을 사용하여 오차를 최소화하는 `Δθ`를 구할 수 있다.
    ...
    ![inverse_kinematics_iterative_6][def11]  
      ...
      ![inverse_kinematics_iterative_9][def14]  
...
- Inverse Kinematics의 Ambiguity
  ...
  ![inverse_kinematics_ambiguity_multiple][def15]

  - Solution이 무한히 많을 수 있다.  
  ![inverse_kinematics_ambiguity_infinite][def16]  

  - Solution이 존재하지 않을 수 있다.  
  ![inverse_kinematics_ambiguity_none][def17]  

- 일반적으로, DoF(자유도, Configuration space)의 수가 Constraints(제약조건, Work space)의 수보다 큰 경우 **ill-posed** 문제가 발생한다.  

  - 이런 경우, 추가적인 제약조건을 도입하여 해결할 수 있다.  
    - Reference Pose `θ`<sub>`0`</sub>으로부터 최소한의 변화량을 가지는 solution을 찾는 방법  
    ![inverse_kinematics_additional_constraints_1][def18]  

    - Naturalness `g(θ)`를 추가하는 방법  
    ![inverse_kinematics_additional_constraints_2][def19]  
...
[def14]: https://i.imgur.com/VaZHgIO.png
[def15]: https://i.imgur.com/lBzv3uy.png
[def16]: https://i.imgur.com/QYn2yaF.png
[def17]: https://i.imgur.com/WeW57aD.png
[def18]: https://i.imgur.com/V7eOylF.png
[def19]: https://i.imgur.com/OSMLpiQ.png