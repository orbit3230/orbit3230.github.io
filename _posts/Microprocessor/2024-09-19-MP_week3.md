...
excerpt: "어셈블리 문법, 분기 명령어, 비교 명령어, 시프트 연산 명령어, 유효주소 저장(포인터) 명령어"
...
- 간접 어드레싱 모드 : 지정한 레지스터의 값에 offset를 더한 것을 메모리 주소로 사용하는 방식
...
- LDR 명령어 시각화  
![LDR_visualization](TODO)  

- STR 명령어 시각화  
![STR_visualization](TODO)  
...
### 6. 문자 입출력 및 프로그램 종료
- 입출력, 프로세스 생성 등은 운영체제의 기능을 호출해야 한다.  

- `SWI`(SoftWare Interrupt) 명령어
  - 운영체제 기능을 호출할 때 사용한다.  
  - `SWI` 명령어를 실행하면 운영체제 모드로 바뀐다.  
  - 특정 기능마다 지정된 번호로 호출한다. (like 리모컨)
