...
excerpt: "Data Format, MIPS-32 ISA, MIPS Design Principles, MIPS Instructions, (Review+) Integers, Representing Instructions, Logical Operations, Review+: From Assembly to Machine Code, MIPS Control Flow Instructions, Procedure, Character Data, Byte/Halfword Operations, Constants, Addressing, MIPS Multiplication and Division"
...
- Leaf Procedure Example  
  ...
  - 컴파일된 MIPS code  
  ![leaf_procedure_example][def31]  

  - Stack & `$sp` in Leaf Procedure Example  
  ![stack_sp_leaf_procedure_example][def32]  
...
- Non-leaf Procecures
  ...
  - 컴파일된 MIPS code  
  ![non_leaf_procedure_example][def33]  

<br>

- Local data on the Stack  
![local_data_on_stack][def34]  
...
![memory_layout][def35]  
  - Text : 프로그램 코드
  - Static data : 전역 변수
    - e.g., C의 static 변수, 상수 배열, 문자열
    - `$gp`는 addressing 하기 위해 이 segment에 ±offset을 사용하여 initialized
  - Dynamic data : heap
    - e.g., C의 `malloc`, Java의 `new`  
  - Stack: automatic storage

<br>

- Example C Program  
![example_c_program][def36]  

- From Program to Process 
![from_program_to_process][def37]  
...
### 12. Byte/Halfword Operations
- Bitwise 연산을 사용할 수 있다.
- MIPS byte/halfword load/store
  - String 처리가 common case
    - Sign extension (32 bits in `rt`)  
    ![byte_halfword_load_store][def38]  
    - Zero extension (32 bits in `rt`)  
    ![byte_halfword_load_store_zero_extension][def39]  
    - 오직 rightmost byte/halfword 저장  
    ![byte_halfword_store][def40]  

  - `strcpy` 예제  
  ![strcpy_example][def41]  
...
- Loading a 32-bit Constant
  - 대부분의 Constants들은 작다.
    - 16-bit immediate면 충분하다.  
    ![i_format_constant][def42]  

  - 하지만 종종 32-bit constant가 필요할 때가 있다.  
  `0000 0000 0011 1101 0000 1001 0000 0000`  

  - 컴파일된 MIPS code  
  ![loading_32_bit_constant][def43]  
...
- Branch Addressing
  ...
  - I-format Branch 명령어  
  ![branch_addressing][def44]  

  - 대부분의 branch targets은 근처에 있다. (forward or backward)

  - PC-relative addressing
    - `Target address` = `Register` + `Branch address`
      - `PC` = `PC` + (`offset` << `2`)
      - `PC`는 이 때 이미 `4`만큼 증가되어 있음
...
-  - Jump targets은 text segment의 어디든 될 수 있다.
    - 명령어 내에서 full address를 인코딩  
    ![jump_addressing][def45]  

  - Direct jump addressing  
    - Target address = PC[31:28] : (address << 2)  

  - Pseudo-direct jump example  
  ![pseudo_direct_jump_example][def46]  

  - Target addressing example  
  ![target_addressing_example][def47]  
    - 만약 `j  Loop` instruction의 주소가 `0x22280024`,  
    `Loop` label의 주소가 `0x22280000` 이라면,  
    `0x8A0000` 으로 인코딩 된다.  
    첫 4bit인 `0010`은 디코딩 시 자동으로 붙여진다.  

- Branching Far Away
  - 만약 branch destination이 16-bit offset 범위를 벗어난다면?
  - 어셈블러가 도와주러 온다.
    - 어셈블러는 branch target으로의 unconditional jump를 삽입하고, condition을 변환한다.  
    ![branching_far_away][def48]  

<br>

- MIPS Addressing Modes  
![mips_addressing_modes][def49]  

<br>

### 15. MIPS Multiplication and Division
- MIPS Multiplication
  - 곱셈 연산에 두 개의 32-bit 레지스터 사용
  ...
  - 명령어
    ...
    - HI/LO에서 `rd`로 이동 (HI 값이 32-bit를 초과하는 지 테스트하는데 유용)  
    `mfhi  rd` / `mflo  rd`
    - Least-significant 32 bits of product -> `rd`  
    `mul  rd, rs, rt`  

  - Instructions example  
  ![mips_multiplication_example][def50]  
...
- MIPS Division
  ...
  `div rs, rt` / `divu rs, rt`
    ...
    - 오버플로우나 divide-by-zero 체크가 없다.
      - 소프트웨어는 필요 시 반드시 체크해야 한다.
    - 결과 액세스에 `mfhi`, `mflo` 사용

  - Instructions example  
  ![mips_division_example][def51]  
...
[def31]: https://i.imgur.com/HWKSeEs.png
[def32]: https://i.imgur.com/sthH2yZ.png
[def33]: https://i.imgur.com/7dF2cxx.png
[def34]: https://i.imgur.com/kkZB4rg.png
[def35]: https://i.imgur.com/Tzbn6d9.png
[def36]: https://i.imgur.com/GxOEcKn.png
[def37]: https://i.imgur.com/KUKbO8e.png
[def38]: https://i.imgur.com/cdjAzIA.png
[def39]: https://i.imgur.com/oG6SGW9.png
[def40]: https://i.imgur.com/tWvEi0l.png
[def41]: https://i.imgur.com/TVrNYi9.png
[def42]: https://i.imgur.com/kMkv0Yw.png
[def43]: https://i.imgur.com/Z7asfHN.png
[def44]: https://i.imgur.com/Aa6PzDF.png
[def45]: https://i.imgur.com/DYQZmZK.png
[def46]: https://i.imgur.com/QcofU6b.png
[def47]: https://i.imgur.com/6csmHmX.png
[def48]: https://i.imgur.com/i5xWlfx.png
[def49]: https://i.imgur.com/LpY0g8x.png
[def50]: https://i.imgur.com/aZFtAWI.png
[def51]: https://i.imgur.com/Yj2thuM.png