...
excerpt: "Introduction, Instruction Processing, MIPS Instructions Design, Single-Cycle Datapath for Arithmetic and Logical Instructions, Single-Cycle Datapath for Data Movement Instructions, Single-Cycle Datapath for Control-Flow Instructions, Datapath for All MIPS Instructions, Single-Cycle Control Unit Design, ALU Control, Example: Single-Cycle Datapath: LW, Single Cycle Microarchitecture Summary, Single-Cycle Microarchitecture Analysis"
...
### 10. ALU Control
- case `opcode`
  - `0` : `funct`에 따라 operation 결정
  - `ALUi` : `opcode`에 따라 operation 결정
  - `LW` : select addition
  - `SW` : select addition
...
- Example ALU operations
  ...
  - `bcond` on equal, not equal, LE zero, GT zero, ...

<br>

- R-type ALU  
![r_type_alu_control][def37]  

- I-type ALU  
![i_type_alu_control][def38]  

- LW  
![lw_alu_control][def39]  

- SW  
![sw_alu_control][def40]  

- Branch (Not Taken)  
![branch_not_taken_alu_control][def41]  

- Branch (Taken)  
![branch_taken_alu_control][def42]  

- Jump  
![jump_alu_control][def43]  

<br>

- Control Signal Summary (Truth Table)  
![control_signal_summary][def44]  

<br>

### 11. Example: Single-Cycle Datapath: LW  
...
### 12. Single Cycle Microarchitecture Summary
- Is this a good idea/design?
  - 하드웨어 관점에서는 단순하고 이해하기 쉬움

- When is this a good design?
  - 명령어가 모두 비슷한 실행 시간을 가질 때

- When is this a bad design?
  - 어떤 명령어는 특정 logical component를 사용하지 않는데,  
  다른 명령어가 내가 사용하지 않는 component를 사용중인 경우 비효율적  
  즉, 가장 느린 명령어가 전체 사이클 타임을 결정짓는 경우 비효율적  

<br>

### 13. Single-Cycle Microarchitecture Analysis  
- Analysis
  - 모든 명령어는 실행하는 데 하나의 사이클을 소요 (CPI = 1 (strictly))
  - 가장 느린 명령어가 실행되는 시간에 각 명령어가 수행되는 시간이 결정된다.
  - 즉, Microarchitecture의 Clock Cycle Time은 가장 느린 명령어가 얼마나 오래 걸리는지에 의해 결정된다.  
  ![single_cycle_analysis][def51]  
    - Memory units (read or write) : `200ps`
    - ALU and adders : `100ps`
    - register file (read or write) : `50ps`
    - other combinational logic : `0ps`  

- Let's find the critical path  
![single_cycle_critical_path][def52]  
  - R-Type & I-Type ALU  
  ![single_cycle_r_type_critical_path][def53]  
  - LW  
  ![single_cycle_lw_critical_path][def54]  
  - SW  
  ![single_cycle_sw_critical_path][def55]  
  - Branch Taken  
  ![single_cycle_branch_taken_critical_path][def56]  
  - Jump  
  ![single_cycle_jump_critical_path][def57]  

<br>

- Single Cycle uArch: Complexity
  - Contrived : 모든 명령어는 가장 느린 명령어 만큼 느리게 실행됨
  - Inefficient
    - 모든 명령어는 가장 느린 명령어 만큼 느리게 실행됨
    - 반드시 worst-case combinational 리소스를 제공해야 함
    - 필요 시, 명령어 processing cycle의 다른 일부 파트 도중 한 번 이상의 리소스 복제가 필요함
  - 꼭 ISA를 구현하는 가장 간단한 방법은 아님
  - 성능 최적화/향상이 쉽지 않음
    - common case 최적화가 효과가 없음
    - 항상 worst-case를 최적화 해야 함  
...
[def37]: https://i.imgur.com/NKfy9Ca.png
[def38]: https://i.imgur.com/ZGU3CxM.png
[def39]: https://i.imgur.com/ArB4eKT.png
[def40]: https://i.imgur.com/zjoz18A.png
[def41]: https://i.imgur.com/4ia7HpT.png
[def42]: https://i.imgur.com/YB61CFm.png
[def43]: https://i.imgur.com/nmV3hzO.png
[def44]: https://i.imgur.com/zHVych8.png
[def45]: https://i.imgur.com/AK4nFx6.png
[def46]: https://i.imgur.com/GHsUBp9.png
[def47]: https://i.imgur.com/144mzCd.png
[def48]: https://i.imgur.com/SjLyr7j.png
[def49]: https://i.imgur.com/lplcz9J.png
[def50]: https://i.imgur.com/yBXoBsR.png
[def51]: https://i.imgur.com/vkCCPWG.png
[def52]: https://i.imgur.com/daENcO5.png
[def53]: https://i.imgur.com/rnGDCcK.png
[def54]: https://i.imgur.com/mQr9u52.png
[def55]: https://i.imgur.com/3EpYa6D.png
[def56]: https://i.imgur.com/iVhuoK6.png
[def57]: https://i.imgur.com/dVTaJPy.png