...
이때, ![differential_definition](TODO)가 성립하면 함수 `f`는 벡터 `a`에서 **미분가능**(differentiable)하다고 한다. 단, 여기서 각 `i`에 대하여 `ε`<sub>`i`</sub>는 `Δx`<sub>`1`</sub>, ..., `Δx`<sub>`n`</sub>에 관한 함수이고, `Δx` → `0`일 때 `ε`<sub>`i`</sub> → `0`을 만족한다.
