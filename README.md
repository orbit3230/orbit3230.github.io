...
- 연도별 탐색의 연도 타임라인을 선택하면 월별 글 분포가 나타납니다. 월을 선택하면 해당 글만 표시하며 `?month=09#2026`처럼 주소에 선택 상태가 유지됩니다.
- **빠른 검색** 버튼이나 `Ctrl/Cmd + K`로 제목·태그·본문을 검색합니다. Esc로 닫고 Tab·Enter로 결과를 탐색합니다.
- 목록의 **미리보기**는 선택한 글만 불러와 요약과 목차를 표시합니다. 가져온 HTML을 실행하거나 삽입하지 않고 텍스트와 내부 링크만 표시합니다.
- **이어 읽기**는 마지막으로 읽던 글 하나의 주소·제목·본문 진행률을 `orbit-reading-resume-v1`에, 글자 크기·줄 간격은 `orbit-reading-settings-v1`에 저장합니다. 서버로 보내지 않으며 같은 기기·브라우저·사이트 주소에서만 유지됩니다. 기록은 90일 후 무시되며, 끝까지 읽거나 안내의 지우기 버튼을 누르면 삭제됩니다. 저장 공간이 차단되어도 현재 페이지의 읽기 기능은 사용할 수 있습니다.
- **집중해서 읽기**는 버튼이나 Esc로 해제합니다. 보조 패널은 화면에서만 접어 플레이어를 재생성하지 않습니다. 코드 복사는 브라우저가 클립보드 접근을 허용하는 HTTPS 또는 localhost 환경에서 작동합니다. 넓은 표는 좌우 스크롤 안내를 표시합니다.

관련 파일은 `assets/site.js`(타임라인·월 필터), `_includes/discovery.html`과 `assets/discovery.js`(검색·미리보기), `assets/reading-tools.js`(읽기 도구), `assets/reading-tools.css`(스타일)입니다.
